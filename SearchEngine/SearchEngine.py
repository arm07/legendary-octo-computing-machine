...
#
# a different way to limit the pages that it can crawl.
#
...
'''
    ...
    depth=0
    while tocrawl and depth<=max_depth:
        links_in_depth=tocrawl[depth]
        tocrawl.append([])
        while links_in_depth:
            page = links_in_depth.pop()
            if page not in crawled:
                union(tocrawl[depth], get_all_links(get_page(page)))
                crawled.append(page)
            depth=depth+1
    return crawled
'''
